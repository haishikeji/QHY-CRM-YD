<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="72%">
    <el-form
      ref="formRef"
...
<!--            <el-input v-model="formData.name" placeholder="请输入客户名称" />-->
            <SearchEnterprises
              ...
              @editConfirm="changeEnterpriseName"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
import SearchEnterprises from './SearchEnterprises.vue'
const { t } = useI18n() // 国际化
...
// 选择 单位回填企业信息
const changeEnterpriseName =  async (val: any) => {
   const res = await CustomerApi.duplicate(val.name)
    if (res.flag==true){
      ElNotification({
        title: '提示',
        message: `（该客户已存在，负责人为${res.owner}）并禁止提交数据。`,
        type: 'error',
      })
      formData.value.name = undefined
      formData.value.creditCode = undefined;
      return
    }else{
      formData.value.name = val.name;
      formData.value.creditCode = val.creditCode;
   }
};
